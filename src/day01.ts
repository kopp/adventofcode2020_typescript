...
export function product_of_three_elements_summing_up_to(input: number[], expected_sum: number): number {
    ...
    for (let index_large_value = sorted_input.length - 1; index_large_value > 2; --index_large_value) {
        for (let index_small_value = 0; index_small_value < index_large_value - 1; ++index_small_value) {
            const partial_sum = sorted_input[index_large_value] + sorted_input[index_small_value];
            if (partial_sum > expected_sum) {
                // console.debug("Skipping because of partial sum ", partial_sum);
                continue;
            }
            let index_intermediate_value = Math.floor((index_large_value + index_small_value) / 2);
            let intermediates_tried: number[] = [];
            while ((index_intermediate_value > index_small_value) && (index_intermediate_value < index_large_value)) {
                const current_sum = sorted_input[index_intermediate_value] + partial_sum;
...
if (require.main === module) {
    const data = read_file_of_numbers("input/day01");
